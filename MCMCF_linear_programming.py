...
def linear_programming_MCMCF(nodes_balances,edges,edge_costs,edge_capacities,nodes,commodities):
    ...
    model.i=[edge[0] for edge in edges ]
    model.j=[edge[1] for edge in edges ]
    ...
    for k in commodities:
        for n in nodes:
            ...
            model.balances_constraint.add(
                (sum(model.x[arch[0], arch[1],k] for arch in entering_edges) - sum(model.x[arch[0], arch[1],k] for arch in exiting_edges)) == (nodes_balances[n][k]))
...
if __name__ == '__main__':
    ...
    linear_programming_MCMCF(nodes_balances,edges,edge_costs,edge_capacities,range(NODE_COUNT),range(COMMODITY_COUNT))
